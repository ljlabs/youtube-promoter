...
class NewVidTweet:
    ...
    def process(self):
        ...
        all_videos = get_all_videos(api_key=self.params.youtube_key,
                                    channel_id=self.channel_upload_id)
        ch = CouchHandler(server_url=self.params.couch_db_url,
                          db_name=self.params.couch_db_table)
